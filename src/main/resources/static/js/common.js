/**
 * 打开页面
 * tabControl tabControl控件
 * caption 新页签标题
 * icon 新页签图标
 * path 页面路径
 */
/*window.openTab = function (tabControl, caption, icon, path) {
	if (!path) {
		return;
	}
 ...
 * path 页面路径
 */
/*window.refreshTab = function (tabControl, path) {
	var tab = tabControl.getTab(path);
	if (tab) {
		tab.getControl().reload();
	}
 ...
 * tabControl tabControl控件
 */
window.closeTab = function (tabControl) {
